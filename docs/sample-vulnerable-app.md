...
## Динамический флаг (CTF-style)

Для official pentest task платформа при каждом запуске стенда генерирует уникальный флаг формата:

```text
pep{[a-zA-Z0-9]{20}}
```

Флаг передается в контейнер как переменная окружения:

```text
PEP_DYNAMIC_FLAG
```

Рекомендация для шаблона задачи:

- не хардкодить флаг в коде и репозитории;
- читать флаг только из `process.env.PEP_DYNAMIC_FLAG` (или аналог в вашем языке);
- отдавать флаг только при успешной эксплуатации уязвимого сценария;
- после получения флага студент отправляет его в API платформы, и лабораторная автоматически отмечается выполненной.
