...
3. Проверить наличие курса Docker и курса `Академия web-безопасности`.
4. Проверить, что Академия содержит 10 практических модулей.
5. Открыть первый модуль и его Markdown-страницу.
6. Проверить дерево разделов и навигацию между модулями.

Ожидаемый результат: курсы и уроки доступны, материалы отображаются на русском языке.

### TC-A02. Docker image submission и technical validation

Шаги:

1. Студент загружает архив стенда или отправляет image reference, port и health path.
...
3. Curator/Admin экспортирует оценки CSV.

Ожидаемый результат: black box отчет сохраняется, CSV содержит студентов и статусы.
...
### TC-A09. Системные стенды

Шаги:

1. Администратор создает системную задачу.
2. У задачи есть корректный архив или образ для запуска.
3. Администратор публикует задачу в каталоге.
4. Студент открывает раздел `Стенды`.
5. Студент запускает стенд на 4 часа.

Ожидаемый результат: задача появляется в каталоге, image собирается в local registry, экземпляр стенда
получает URL вида `task-<slug>-<id>.127.0.0.1.nip.io:8088`.
...
3. Открыть каталог курсов.
4. Выбрать модуль, проверить Markdown-материал и дерево разделов.
