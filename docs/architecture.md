...
- страницы Академии web-безопасности;
- форма сдачи архива стенда или Docker image reference;
- каталог официальных системных стендов;
...
Worker проверяет готовые Docker images и archive submissions: собирает стенд, публикует image в local
registry, запускает временный контейнер и проверяет port/health. Для системных задач worker-контур
использует подготовленный архив/образ и подготавливает runtime image для `kind`.
