...
- Запуск лабораторий: локальный Kubernetes через `kind`
...
## Запуск lab runtime через kind

Для демонстрации запуска уязвимых приложений используется локальный Kubernetes через `kind`.
